...
- `bin/kanban` 的 `start` 未传 `--agent` 时读取生效的 `kanban_agent`; `--agent` 始终优先. `--launcher` 可覆盖本次启动且不改机器配置; launcher 为 `tmux` 时沿用独立 window, 为 `foreground` 时必须有交互 tty 并在当前终端等待 Agent 退出. `start --all` 批量启动全部 `todo` 卡 (只支持 tmux, 受 `max_concurrent_tasks` 约束), `--limit N` 限制数量. 验收确认按原始交互式设计: 执行 Agent (llm-agent) 任务完成后停在窗口等待用户输入确认词 (验收通过/确认/y), 收到后自动填卡 `move done`; 拒绝则保留 `working/`.
